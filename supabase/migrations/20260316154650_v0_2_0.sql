...
-- ----------------------------------------
CREATE OR REPLACE FUNCTION add_owner_to_project_members () RETURNS trigger AS $$
BEGIN
...
CREATE POLICY "authenticated users can read profiles" ON profiles FOR
SELECT
...
CREATE POLICY "project members can read" ON projects FOR
SELECT
  USING (is_project_member (id));

CREATE POLICY "owner can create" ON projects FOR INSERT
WITH
  CHECK (
    (
      SELECT
...
CREATE POLICY "owner can delete" ON projects FOR DELETE USING (
  (
    SELECT
      auth.uid ()
  ) = owner_id
...
-- ----------------------------------------
CREATE POLICY "project members can read members" ON project_members FOR
SELECT
  USING (is_project_member (project_id));

CREATE POLICY "owner can manage members" ON project_members FOR INSERT
WITH
  CHECK (is_project_owner (project_id));

CREATE POLICY "owner can delete members" ON project_members FOR DELETE USING (is_project_owner (project_id));
...
CREATE POLICY "project members can read tickets" ON tickets FOR
SELECT
  USING (is_project_member (project_id));

CREATE POLICY "project members can create tickets" ON tickets FOR INSERT
WITH
  CHECK (is_project_member (project_id));

CREATE POLICY "project members can update tickets" ON tickets
FOR UPDATE
  USING (is_project_member (project_id));

CREATE POLICY "project members can delete tickets" ON tickets FOR DELETE USING (is_project_member (project_id));
